...
import { OtpConfig, OtpPlatform, OtpRequestLog, PLATFORM_PRESETS } from './entities/otp-config.entity';
...
export class OtpRelayService {
  ...
  async getConfigs(tenantId: string, storeId: string) {
  ...
  async createConfig(tenantId: string, storeId: string, data: any) {
  ...
  async getAnalytics(id: string, tenantId: string, days = 7) {
    const config = await this.getConfig(id, tenantId);
    const since = new Date(); since.setDate(since.getDate() - days);

    const logs = await this.logRepo.find({
      where: { configId: id, createdAt: MoreThan(since) },
    ...
    return {
      config: { totalViews: config.totalViews, totalRequests: config.totalRequests, successCount: config.successCount, failCount: config.failCount, successRate: config.totalRequests > 0 ? Math.round(config.successCount / config.totalRequests * 100) : 0 },
  ...
  async getPublicPage(slug: string) {
    const c = await this.configRepo.findOne({ where: { slug, isActive: true } });
    if (!c) return null;
    ...
    return {
      ...
      bgColor: c.bgColor, primaryColor: c.primaryColor, successMsg: c.successMsg,
      noCodeMsg: c.noCodeMsg, needsUsername: c.needsUsername, usernameLabel: c.usernameLabel,
      buttonText: c.buttonText, platform: c.platform,
      platformLabel: preset?.label || c.platform, platformIcon: preset?.icon || '🔑',
      verifyOrder: c.verifyOrder,
    };
  }

  async requestOtp(slug: string, orderNumber: string, username: string, clientIp: string) {
    ...
    try {
      // ✅ التحقق من الطلب — مربوط بمتجر التاجر فقط (tenant isolation)
      if (c.verifyOrder && orderNumber) {
        ...
        if (!store?.accessToken || store.platform !== 'salla') {
          throw new BadRequestException('تعذر التحقق من الطلب — إعدادات المتجر غير مكتملة');
        }

        const token = decrypt(store.accessToken);
        if (!token) throw new BadRequestException('تعذر التحقق — مشكلة في بيانات المتجر');

        // البحث عن الطلب في متجر هذا التاجر فقط
        ...
        this.logger.log(`🔑 ✅ Order verified: ref=${order.reference_id} in store ${c.storeId}`);
      }
      ...
      if (!code) {
        log.success = false;
        await this.configRepo.increment({ id: c.id }, 'failCount', 1);
        log.responseMs = Date.now() - start;
        await this.logRepo.save(this.logRepo.create(log));
        // ✅ رسالة للعميل: أعد إرسال الرمز
  ...
  private openImap(host: string, port: number, user: string, pw: string, tls: boolean): Promise<Imap> {
  ...
  private async extractOtp(config: OtpConfig, pw: string): Promise<string | null> {
    ...
    try {
      ...
      await new Promise<void>((res, rej) => { imap!.openBox('INBOX', true, err => err ? rej(err) : res()); });

      // ✅ فقط رسائل آخر X دقائق (freshness)
      ...
      const uids = await new Promise<number[]>((res, rej) => {
        imap!.search(criteria, (err, r) => err ? rej(err) : res(r || []));
      });

      if (uids.length === 0) {
        this.logger.debug(`🔑 No fresh emails (last ${config.freshnessMinutes}min) for ${config.platform}`);
        imap.end();
        return null;
      }
      ...
      const email = await new Promise<any>((resolve, reject) => {
        ...
        f.once('error', err => { if (!done) reject(err); });
      ...
      // ✅ التحقق من الوقت — فقط آخر freshnessMinutes دقائق
      if (email.date) {
        const emailAge = (Date.now() - new Date(email.date).getTime()) / 60000;
        if (emailAge > (config.freshnessMinutes || 3)) {
          this.logger.debug(`🔑 Email too old: ${emailAge.toFixed(1)}min > ${config.freshnessMinutes}min`);
          return null;
        }
      }

      const body = (email.text || '') + ' ' + (email.html || '');
      const regex = config.otpRegex ? new RegExp(config.otpRegex, 'i') : new RegExp(PLATFORM_PRESETS[config.platform]?.otpRegex || '([A-Z0-9]{4,8})', 'i');
  ...
  private checkRate(ip: string, limit: number) {
    const now = Date.now();
